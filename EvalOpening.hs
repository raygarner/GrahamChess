...
totalMaterial c ps = sum [pieceVal (y,White,(0,0),0) * countPieceType c y ps  | y <- pieceTypes]
...
totalMobility c ps = sum [ evalPiece x ps | x <- ps, (getPieceType x == Knight || getPieceType x == Bishop), getColour x == c, getPos x /= (-1,-1) ]
...
totalOpeningValColour c ps = totalMaterial c ps + totalMobility c ps + kingSafety c ps
...
movePieceBonus c ps = fromIntegral (length [x | x <- ps, getPos x /= (-1,-1), getColour x == c, getMovecount x == 0, getPieceType x == Knight || getPieceType x == Bishop]) * (-80)
