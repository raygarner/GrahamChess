...
totalMobility c ps = ( 15 * sum [ evalPiece x ps | x <- ps, getColour x == c, getPos x /= (-1,-1), getPieceType x /= Queen]) - (20 * sum [ evalPiece y ps | y <- ps, getColour y /= c, getPos y /= (-1,-1), getPieceType y /= Queen])
