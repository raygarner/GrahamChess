...
middleMoveWrapper :: Int -> Colour -> AllPieces -> (Piece, Move, Float)
middleMoveWrapper d c ps = findStrongestMoveFromAll c (par s4 (par s3 (par s2 (s1:s2:s3:s4:[]))))
                            where
                                a = makeEvalList c ps
                                e = length a
                                l = take ((e `div` 2)) a
...
findMostEpicMove (a,b) 0 c ps xs = ((King,White,(0,0),0),(0,0),fromIntegral (totalMiddleVal ps))
